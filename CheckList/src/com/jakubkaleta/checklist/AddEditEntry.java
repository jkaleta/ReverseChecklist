 ...
 * @author Jakub Kaleta
 */
public class AddEditEntry extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		if (mode.equalsIgnoreCase("EDIT"))
		{
			...
			Cursor mCursor = managedQuery(EntryColumns.CONTENT_URI, ENTRY_PROJECTION,
					EntryColumns.TABLE_NAME + "." + EntryColumns._ID + " = " + entry_id, null,
					EntryColumns.DEFAULT_SORT_ORDER);

			mCursor.moveToFirst();

			entryName.setText(mCursor.getString(mCursor.getColumnIndex(EntryColumns.ENTRY_NAME)));
			category_id = mCursor.getLong(mCursor.getColumnIndex(EntryColumns.CATEGORY_ID));
		}
		else
		{
	...
	public void onPostCreate(Bundle savedInstanceState)
	{
		...
		addEditEntry.setOnClickListener(new View.OnClickListener()
		{
			public void onClick(View view)
			{
				...
				intentToPassBack.putExtra(EntryColumns.CATEGORY_ID, spinnerCategories
						.getSelectedItemId());
		...
		entryName.addTextChangedListener(new TextWatcher()
		{
			...
			public void onTextChanged(CharSequence s, int start, int before, int count)
			{
				...
				{
					String text = entryName.getText().toString().replace(";", "");
					addEditEntry.setEnabled(!spinnerCategories.getAdapter().isEmpty()
							&& text.length() > 0);
				}
	...
	protected void onResume()
	{
		...
		if (!categoriesExist)
		{
			...
			dialogBuilder.setTitle(R.string.add_category).setMessage(
					R.string.add_item_no_categories_defined).setIcon(R.drawable.appicon)
	...
	private void bindCategoryDropdown()
	{
		...
		Cursor mCursor = managedQuery(CategoryColumns.CONTENT_URI, CATEGORY_PROJECTION,
				CategoryColumns.ACTIVITY_ID + " = " + activity_id, null,
				CategoryColumns.TABLE_NAME + "." + CategoryColumns.SORT_POSITION);

		SimpleCursorAdapter adapter = new SimpleCursorAdapter(this,
				android.R.layout.simple_spinner_item, mCursor, new String[]
				{ CategoryColumns.CATEGORY_NAME, CategoryColumns._ID }, new int[]
				{ android.R.id.text1 });
		...
		// if there were no categories for the selected activity, the category
		// spinner
		// must be disabled. We don't want users adding entries with no category
		// selected.
